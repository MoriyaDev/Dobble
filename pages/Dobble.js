...
////A function that shuffles the array like you shuffle a pack of cards
...
// function to create cards with one pair of matching images
...
// }
  ...
  const commonObjectIndex = Math.floor(Math.random() * pictures.length); 
  // Randomly selects a common object location for 2 twin images// בוחר באקראי מיקום אוביקט משותף עבור 2 תמונות תאומות
  let commonFound1 = false; // האם קיים כבר קלף כזהDoes such a card already exist?
  let commonFound2 = false; // האם קיים כבר קלף כזה

  let folder = "../images/";

  // הוסף תמונות ייחודיות לכרטיס 1
  // Add unique images to card 1
  for (let i = 0; i < (numOfImg - 1); i++) {
    ...
    if (!commonFound1 && pictures[i] !== pictures[commonObjectIndex]) {
      ...
      ///if the current image exists, skip it //אם התמונה הנוכחית קיימת, דלג עליה
      commonFound1 = true;
      img.addEventListener("click", check)
      sp.appendChild(img)
      document.getElementById("card1").appendChild(sp);
  ...
  // Add the shared image separately to each card
...
function gameOver() {
  {
    const sound = document.getElementById('oops');
    sound.play();
}
